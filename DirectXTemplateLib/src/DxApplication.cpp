...
typedef std::map< HWND, DxWindow* > WindowMap;
typedef std::map< std::wstring, DxWindow* > WindowNameMap;

// An invalid window to return if an error occurred while creating a window.
DxWindow DxApplication::ms_InvalidWindow;

static DxApplication* gs_pSingelton = nullptr;
static WindowMap gs_Windows;
static WindowNameMap gs_WindowByName;

static LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);

DxApplication::DxApplication(HINSTANCE hInst)
	: m_hInstance(hInst)
{
...
void DxApplication::Create(HINSTANCE hInst)
{
	if (!gs_pSingelton)
	{
		gs_pSingelton = new DxApplication(hInst);
	}
}

DxApplication& DxApplication::Get()
{
	assert(gs_pSingelton);
	return *gs_pSingelton;
}

void DxApplication::Destroy()
{
	if (gs_pSingelton)
	{
...
DxWindow& DxApplication::CreateRenderWindow(const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync, bool windowed)
{
	...
	DxWindow* pWindow = new DxWindow(hWnd, windowName, clientWidth, clientHeight, vSync, windowed);
...
void DxApplication::DestroyWindow(DxWindow& window)
{
	window.Destroy();
}

void DxApplication::DestroyWindow(const std::wstring& windowName)
{
	DestroyWindow(GetWindowByName(windowName));
}

DxWindow& DxApplication::GetWindowByName(const std::wstring& windowName)
{
...
void DxApplication::Quit(int exitCode)
{
...
static void RemoveWindow(HWND hWnd)
{
	...
	{
		DxWindow* pWindow = windowIter->second;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		DxKeyCode::Key key = (DxKeyCode::Key)wParam;
		unsigned int scanCode = (lParam & 0x00FF0000) >> 16;
		DxKeyEventArgs keyEventArgs(key, c, DxKeyEventArgs::Pressed, shift, control, alt);
	...
	case WM_KEYUP:
	{
		...
		DxKeyEventArgs keyEventArgs(key, c, DxKeyEventArgs::Released, shift, control, alt);
