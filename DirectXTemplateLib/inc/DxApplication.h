...
public:
     ...
     * returned.
     */
    DxWindow& CreateRenderWindow( const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync = false, bool windowed = true );
     ...
     */
    void DestroyWindow( DxWindow& window );

    /**
     * Find a window by the window name.
     */
    DxWindow& GetWindowByName( const std::wstring& windowName );
...
private:
    ...
    static DxWindow ms_InvalidWindow;
};