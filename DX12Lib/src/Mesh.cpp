...
//
//const D3D12_INPUT_ELEMENT_DESC CubeVertexPositionNormalTexture::InputElements[] =
//{
...
std::unique_ptr<Mesh> Mesh::CreateSphere(CommandList& commandList, float diameter, size_t tessellation, bool rhcoords)
{
	...
	for (size_t i = 0; i <= verticalSegments; i++)
	{
		...
		for (size_t j = 0; j <= horizontalSegments; j++)
		{
			...
			vertices.push_back(CubeVertexPositionNormalTexture(normal * radius, normal, textureCoordinate));
		}
	}
...
std::unique_ptr<Mesh> Mesh::CreateCube(CommandList& commandList, float size, bool rhcoords)
{
	...
	{
		XMVECTOR normal = faceNormals[i];// XMVectorSet(faceNormals[i].x, faceNormals[i].y, faceNormals[i].z, 1.0f);
		...
		vertices.push_back(CubeVertexPositionNormalTexture(pos, norm, textureCoordinates[0], i));
		XMStoreFloat3(&pos, (normal - side1 + side2) * size);
		vertices.push_back(CubeVertexPositionNormalTexture(pos, norm, textureCoordinates[1], i));
		XMStoreFloat3(&pos, (normal + side1 + side2) * size);
		vertices.push_back(CubeVertexPositionNormalTexture(pos, norm, textureCoordinates[2], i));
		XMStoreFloat3(&pos, (normal + side1 - side2) * size);
		vertices.push_back(CubeVertexPositionNormalTexture(pos, norm, textureCoordinates[3], i));
	}
...
static void CreateCylinderCap(CubeVertexCollection& vertices, IndexCollection& indices, size_t tessellation, float height, float radius, bool isTop)
{
	// Create cap indices.
	...
	for (size_t i = 0; i < tessellation; i++)
	{
		...
		vertices.push_back(CubeVertexPositionNormalTexture(position, normal, textureCoordinate));
	}
}
...
std::unique_ptr<Mesh> Mesh::CreateCone(CommandList& commandList, float diameter, float height, size_t tessellation, bool rhcoords)
{
	...
	for (size_t i = 0; i <= tessellation; i++)
	{
		...
		vertices.push_back(CubeVertexPositionNormalTexture(topOffset, normal, g_XMZero));
		vertices.push_back(CubeVertexPositionNormalTexture(pt, normal, textureCoordinate + g_XMIdentityR1));
...
template<typename T>
static void ReverseWinding(IndexCollection& indices, std::vector<T>& vertices)
{
...
template<typename T>
void Mesh::Initialize(CommandList& commandList, std::vector<T>& vertices, IndexCollection& indices, bool rhcoords)
{
	...
	m_IndexBuffer.SetName(L"Indices");
