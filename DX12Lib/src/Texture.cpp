...
namespace File {
	uint8_t* LoadTextureDataFromFile(const wchar_t* fileName, ID3D12Resource** ppTex) {
		std::unique_ptr<uint8_t[]> decodedData;
		D3D12_SUBRESOURCE_DATA subresource;
		HRESULT hr = DirectX::LoadWICTextureFromFile(Application::Get().GetDevice().Get(), fileName, ppTex, decodedData, subresource);

		return decodedData.get();
	}
}